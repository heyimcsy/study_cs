...
* [보조기억장치와 입출력장치](#보조기억장치와-입출력장치)
  ...
  * [입출력 기법](#입출력-기법)
    ...
    * [인터럽트 기반 입출력: 다중 인터럽트](#인터럽트-기반-입출력-다중-인터럽트)
      * [PIC: 다중 인터럽트를 처리하기 위해서는 **프로그래머블 인터럽트 컨트롤러**(PIC)라는 하드웨어가 사용된다.](#pic-다중-인터럽트를-처리하기-위해서는-프로그래머블-인터럽트-컨트롤러pic라는-하드웨어가-사용된다)
    * [DMA 입출력](#dma-입출력)
      * [DMA: 직접 메모리에 접근할 수 있는 입출력 기능 (Direct memory access)](#dma-직접-메모리에-접근할-수-있는-입출력-기능-direct-memory-access)
      * [PCIe: 대표적인 입출력 버스로는 PCIe가 있다. PCI 라는 입출력 버스의 발전된 형태로, 오늘날 메인 보드에서 가장 대중적으로 볼 수 있는 입출력 버스이다.](#pcie-대표적인-입출력-버스로는-pcie가-있다-pci-라는-입출력-버스의-발전된-형태로-오늘날-메인-보드에서-가장-대중적으로-볼-수-있는-입출력-버스이다)
<!-- TOC -->
...
### DMA 입출력
- 프로그램 기반의 입출력과 인터럽트 기반의 입출력의 공통점은 CPU가 입출력 장치와 메모리간의 데이터 이동을 주도해야 하며, 이동하는 데이터들도 반드시 CPU를 거친다는 것
  - 입출력 장치의 데이터를 메모리에 저장하는 경우 1. 장치 컨트롤러로부터 데이터를 하나씩 읽어 레지스터에 적재 2. 적재한 데이터를 하나씩 메모리에 저장
  - 메모리 속 데이터를 입출력장치에 내보내는 경우 1. 메모리로부터 데이터를 하나씩 읽어 레지스터에 적재 2. 적재한 데이터를 하나씩 입출력장치에 내보낸다.
- 위의 두 경우처럼 모든 데이터가 CPU를 거쳐야 하면, CPU의 부담이 커진다. 그래서 CPU를 거치지 않고 입출력장치와 메모리가 상호작용 가능한 **DMA**가 등장
#### DMA: 직접 메모리에 접근할 수 있는 입출력 기능 (Direct memory access)
- DMA 입출력을 위해 시스템 버스에 연결된 DMA 컨트롤러라는 하드웨어가 필요하다. 
- DMA 컨트롤러는 시스템 버스에 연결되도, 입출력장치들의 장치 컨트롤러들은 입출력 버스라는 전용 버스와 연결된다. 
- 일반적인 DMA 입출력 과정
1. CPU가 DMA 컨트롤러에게 `입출력장치의 주소, 수행할 연산, 연산할 메모리 주소 등의 정보와 함께` 입출력 작업을 명령한다. 
2. DMA 컨트롤러가 CPU대신 장치 컨트롤러와 상호작용하며 입출력 작업을 수행한다. 필요한 경우 직접 접근해 정보를 읽거나 쓴다. 
3. 입출력 작업이 끝나면 CPU 인터럽트를 걸어 작업이 끝났음을 알린다.

`🐣사이클 스틸링: 버스는 공용자원이다.또한, 두 장치가 동시에 하나의 버스를 이용할 수 없다. CPU입장에서는 DMA 컨트롤러에세 버스에 접근하는 사이클을 도둑 맞는 기분이 들고, 이를 사이클 스틸링이라 한다.`

#### PCIe: 대표적인 입출력 버스로는 PCIe가 있다. PCI 라는 입출력 버스의 발전된 형태로, 오늘날 메인 보드에서 가장 대중적으로 볼 수 있는 입출력 버스이다.
- PCIe 와 연결된 장치의 성능과 관련해 알아야 하는 내용은 
1. PCIe 버전에 따라 최대 속도가 달라질 수 있다. 
  - PCIe 버스는 지속적으로 발전하고 있어 버전이 존재한다. 3.0, 4.0, 5.0 과 같이 숫자를 덧붙인다.
  - 버전 숫자가 클 수록 레인당 최대 속도가 크다.
2. PCIe 버스가 여러 레인을 이용해 정보를 주고받을 수 있다는 것
  - 레인이란 PCIe 버스를 통해 장보를 송수신하는 단위를 말한다.
  - 통신을 주고받는 선로의 수와 같은 개념, 레인이 2,4,8개가 되면 한 번에 통신을 주고받을 수 있는 양도 2, 4,8배가 된다.
  - 예를 들어 PCIE40. * 4는 이론상 1,969MB/s * 4만큼의 속도를 낼 수 있다. 